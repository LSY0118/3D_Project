...
void CGameFramework::OnResizeBackBuffers()
{
	...
	m_pdxgiSwapChain->ResizeBuffers(0, 0, 0, DXGI_FORMAT_UNKOWN, 0);
	m_nSwapChainBufferIndex = 0;
#else
	DXGI_SWAP_CHAIN_DESC dxgiSwapChainDesc;
	m_pdxgiSwapChain->GetDesc(&dxgiSwapChainDesc);
...
void CGameFramework::CreateSwapChain()
{
	...
	m_pdxgiFactory->MakeWindowAssociation(m_hWnd, DXGI_MWA_NO_ALT_ENTER);	// Alt + Enter 키의 동작을 비활성화한다.
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	::CreateDXGIFactory1(__uuidof(IDXGIFactory4), (void**)&m_pdxgiFactory);	// DXGI팩토리를 생성한다.
	...
	m_bMsaa4xEnable = (m_nMsaa4xQualityLevels > 1) ? true : false; //다중 샘플의 품질 수준이 1보다 크면 다중 샘플링을 활성화 한다.
...
void CGameFramework::CreateRtvAndDsvDescriptorHeaps()
{
	...
	m_nRtvDescriptorIncrementSize = m_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);	// 렌더 타겟 서술자 힙의 원소의 크기를 저장한다.

	d3dDescriptorHeapDesc.NumDescriptors = 1;
	d3dDescriptorHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV;
	hResult = m_pd3dDevice->CreateDescriptorHeap(&d3dDescriptorHeapDesc, __uuidof(ID3D12DescriptorHeap), (void**)&m_pd3dDsvDescriptorHeap);	// 깊이-스텐실 서술자 힙(서술자의 개수는 1)을 생성한다.
	m_nDsvDescriptorIncrementSize = m_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);	//깊이-스텐실 수설주ㅏ 힙의 원소의 크기를 저장한다.
}
...
void CGameFramework::FrameAdvance()
{
	...
	m_pd3dCommandList->RSSetScissorRects(1, &m_d3dScissorRect);	//뷰포트와 시저 사각형을 설정한다.

	D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle = m_pd3dRtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	d3dRtvCPUDescriptorHandle.ptr += (m_nSwapChainBufferIndex * m_nRtvDescriptorIncrementSize);	// 현재의 렌더 타겟에는 해당하는 서술자의 CPU 주소(핸들)를 계산한다.

	float pfClearColor[4] = { 0.f, 0.125f, 0.3f, 1.f };
	m_pd3dCommandList->ClearRenderTargetView(d3dRtvCPUDescriptorHandle, pfClearColor, 0, NULL);	// 원하는 색상으로 렌더 타겟(뷰)를 지운다.
	...
	m_pd3dCommandList->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.f, 0, 0, NULL);	// 원하는 값으로 깊이-스텐실(뷰)을 지운다.
...
void CGameFramework::WaitForGpuComplete()
{
	...
	{
		// 펜스의 현재 값이 설정한 값보다 작으면 펜스의 현재 값이 설장한 값이 될 때까지 기다린다.
